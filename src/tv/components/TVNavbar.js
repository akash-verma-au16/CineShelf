...
export default function Navbar() {
	...
	const { loading, scanLibrary } = useApp();
	...
	useEffect(() => {
		if (location.pathname !== '/search' && location.pathname !== '/browse') {
			setSearchQuery('');
		}
	}, [location.pathname]);
	...
	function handleSearch(e) {
		...
		setSearchQuery(q);
		if (q.trim()) {
			navigate(`/browse?workflow=tv&q=${encodeURIComponent(q.trim())}`);
		}
	}
	...
	function handleSearchKey(e) {
		if (e.key === 'Enter' && searchQuery.trim()) {
			navigate(
				`/browse?workflow=tv&q=${encodeURIComponent(searchQuery.trim())}`,
			);
		}
		if (e.key === 'Escape') {
			setSearchQuery('');
	...
	return (
		<nav
			...
			<div className="titlebar-no-drag flex items-center gap-1 text-sm">
				...
				<NavLink
					active={location.pathname === '/anime'}
