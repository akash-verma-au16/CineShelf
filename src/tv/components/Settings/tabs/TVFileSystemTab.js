...
export default function FileSystemTab() {
	...
	const sourceDirs = settings?.sourceDirs || [];

	// nodes: { [path]: { items: [], loaded: bool, loading: bool, expanded: bool, error: null } }
	...
	function toggleNode(dirPath) {
		setNodes((prev) => {
			...
			if (willExpand && !n.loaded && !n.loading) {
				// Load asynchronously after state update
		...
		<div className="px-8 py-8">
			<div className="mb-6">
				...
				<p className="text-sm text-gray-500 mt-1">
					Browse source directories. Hover any item and click the pencil icon to
					rename files and folders directly on disk.
				</p>
			</div>
	...
	return (
		<div>
			<div
						...
						<div>
							<div className="flex items-center gap-1.5">
								<input
									...
									className="input-field text-xs py-0.5 h-6 flex-1 min-w-0"
				...
				{/* File metadata chips — visible on hover */}
				...
				{/* Reload button for directories on hover */}
				...
				{/* Rename button — visible on hover, hidden for root dirs */}
				{!isRenaming && !isRoot && (
					<button
						onClick={(e) => {
							e.stopPropagation();
