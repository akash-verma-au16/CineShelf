...
/** Horizontally scrollable row of series cards */
export default function SeriesRow({
	...
	// Recalculate after series list changes / on mount
	useEffect(() => {
		updateButtons();
	}, [series]);
	...
	function scroll(dir) {
		const el = scrollRef.current;
		if (!el) return;
		// Measure one card + gap so we can keep the last 2 cards in view
		const firstChild = el.children[0];
		const cardW = firstChild
			? firstChild.getBoundingClientRect().width + 12 // 12 px = gap-3
			: 172;
		const visibleCards = Math.floor(el.clientWidth / cardW);
		// Always advance by (visible - 2), minimum 1
		const scrollCards = Math.max(1, visibleCards - 2);
		el.scrollBy({
			left: dir === 'left' ? -(scrollCards * cardW) : scrollCards * cardW,
			behavior: 'smooth',
		});
	}
	...
	return (
		<section className="mb-8 group/row">
			...
			<div className="relative">
				{/* Left scroll button — hidden at start position */}
				{canScrollLeft && (
					<ScrollBtn
						dir="left"
						onClick={() => scroll('left')}
				...
				{/* Cards container — pl-6 gives visible indent at start,
				    cards scroll freely to the left edge once you scroll */}
				<div
					ref={scrollRef}
