...
const EXPAND_W = 156; // extra width revealed on hover

/**
 * Series card that expands horizontally on hover to reveal metadata inline.
 * No popup below — avoids z-index/overflow clipping issues.
 */
export default function SeriesCard({ series, compact = false }) {
	const navigate = useNavigate();
	const { metadata, history, favorites, toggleFavorite } = useApp();
	...
	const watchedCount = allEps.filter((ep) => history[ep.id]?.completed).length;
	const seriesProgress =
		allEps.length > 0 ? Math.round((watchedCount / allEps.length) * 100) : 0;
	...
	const cardH = Math.round(pW * 1.5); // 2:3 poster ratio
	...
	return (
		<div
			...
			{/* Card body — flex row, fixed height */}
			<div
