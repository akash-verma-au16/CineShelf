...
import SeriesCard from './TVSeriesCard';
...
// Card IDs are STABLE (no rowId embedded) so dnd-kit tracks them correctly during cross-row moves
...
// ── AddShowModal ────────────────────────────────────────────────────────────
// allUsedIds = union of seriesIds in ALL rows — filters out already-placed shows
		...
		<div
			...
			<div
				...
				<div className="flex items-center justify-between">
					<h3 className="font-bold text-white text-sm">Add Show to Shelf</h3>
					<button
						onClick={onClose}
				...
				<div className="overflow-y-auto flex flex-col gap-0.5 -mx-1">
					...
					{filtered.length === 0 && (
						<p className="text-gray-600 text-sm text-center py-4">
							{q ? 'No results' : 'All shows already placed in a shelf'}
						</p>
					)}
				</div>
			</div>
...
function EmptyDropZone({ rowId, isOver }) {
	...
	return (
		<div
			...
			{isOver ? 'Drop here' : 'Drag shows here or use + Add Show'}
		</div>
	);
}

// ── DraggableCard ──────────────────────────────────────────────────────────
// group/card MUST be on the outer setNodeRef div so grip handle hover state works
...
function DraggableCard({ series, rowId, isDragActive, onRemove }) {
	...
	return (
		<div
			...
			{/* Grip indicator — decorative only, not a drag handle */}
			<div
			...
			{/* Remove button — sits right next to grip icon, top-left */}
			<button
...
function SortableRow({
	...
	const seriesInRow = row.seriesIds
		...
		.filter(Boolean);
	const cardIds = row.seriesIds.map(cardId); // stable: card::seriesId
			...
			<div className="flex items-center gap-2 pl-4 pr-6 mb-3">
				...
				{seriesInRow.length > 0 && (
					<span className="ml-auto text-xs text-gray-600 opacity-0 group-hover/row:opacity-100 transition-opacity">
						{seriesInRow.length} show{seriesInRow.length !== 1 ? 's' : ''}
					</span>
				)}
			</div>
...
export default function CustomRowsSection() {
	const {
		allSeries,
		metadata,
	...
	// Tracks which row the currently-dragged card is in (updated live on cross-row moves)
	const activeCardRowRef = useRef(null);

	// Persist on change (skip first render to avoid overwriting loaded state)
	...
	// All series IDs already placed in any row
	...
	// Add to target row AND remove from any other row (enforces no duplicates across shelves)
	...
	// Remove from a row (returns it to the available-to-add pool)
	const removeSeriesFrom = useCallback((rowId, seriesId) => {
		setRows((prev) =>
			prev.map((r) =>
				r.id === rowId
					? { ...r, seriesIds: r.seriesIds.filter((id) => id !== seriesId) }
					: r,
			),
		);
	}, []);
	...
	function onDragOver({ active, over }) {
		...
		// Do NOT update rows/cards here. Only visual feedback.
	}
		...
		if (activeData?.type === 'card') {
			...
			if (targetRowId && targetRowId !== currentRowId) {
				setRows((prev) => {
					...
					if (dstRow.seriesIds.includes(activeSeriesId)) return prev; // already moved
			...
			if (overSeriesId && activeSeriesId !== overSeriesId) {
				setRows((prev) => {
					...
					return prev.map((r) =>
						r.id === currentRowId
							? { ...r, seriesIds: arrayMove(r.seriesIds, oldIdx, newIdx) }
							: r,
					);
				});
			}
			...
			<div className="px-6 mb-8">
				<div className="border border-dashed border-white/10 rounded-2xl p-10 flex flex-col items-center gap-4 text-center">
					<p className="text-gray-500 text-sm">
						No custom shelves yet — organise your library your way
					</p>
					<button
						onClick={addRow}
