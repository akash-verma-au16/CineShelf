...
export default function AnimeGeneralTab() {
	const {
		settings,
		saveSettings,
		scanLibrary,
	...
	useEffect(() => {
		if (settings && !form) {
			setForm({
				animeSourceDirs: settings.animeSourceDirs || [],
	...
	async function pickDir() {
		...
		if (dir && !form.animeSourceDirs.includes(dir)) {
			update('animeSourceDirs', [...form.animeSourceDirs, dir]);
		}
	}
	...
	function addManualDir() {
		const d = newDir.trim();
		if (!d) return;
		if (form.animeSourceDirs.includes(d)) {
			showToast('Directory already in list', 'warning');
			return;
		}
		update('animeSourceDirs', [...form.animeSourceDirs, d]);
		setNewDir('');
	}
	...
	function removeDir(dir) {
		update(
			'animeSourceDirs',
			form.animeSourceDirs.filter((d) => d !== dir),
		);
	}

	const isScanning = loading?.scanning;

	return (
		<div className="px-10 py-10 max-w-3xl">
			<h1 className="text-2xl font-bold text-white mb-8">Anime — General</h1>

			{/* Source directories */}
			<Section
				title="Media Source Directories"
				desc="Folders where your anime series are stored. Each sub-folder (or the folder itself) is treated as one series. Filler List JSON files inside each series folder are automatically detected.">
				<div className="space-y-2 mb-3">
					{form.animeSourceDirs.map((dir) => (
						<div
							key={dir}
				...
				<div className="flex gap-2">
					<input
						...
						placeholder="Paste a path manually, e.g. E:\Anime"
			...
			<Section
				...
				<label className="flex items-center gap-3 cursor-pointer">
					<div
						className={`relative w-10 h-5 rounded-full transition-colors duration-200 ${form.autoResume ? 'bg-violet-600' : 'bg-white/20'}`}
			...
			<Section
				...
				<label className="flex items-center gap-3 cursor-pointer">
					<div
						className={`relative w-10 h-5 rounded-full transition-colors duration-200 ${autostart ? 'bg-violet-600' : 'bg-white/20'}`}
			...
			{library && (
				<Section
					...
					<div className="grid grid-cols-4 gap-3">
						<Stat
							label="Series"
							value={library.totalSeries || 0}
						/>
						<Stat
							label="Episodes"
							value={library.totalEpisodes || 0}
						/>
						<Stat
							label="Canon + Mixed"
							value={
								library.series
									? library.series.reduce(
											(n, s) => n + s.canonCount + s.mixedCount,
											0,
										)
									: 0
							}
						...
						<Stat
							...
							value={
								library.series
									? library.series.reduce((n, s) => n + s.fillerCount, 0)
									: 0
							}
						/>
					</div>
			...
			<p className="text-xs text-gray-600 mt-6">
				...
				AppData folder. Metadata is fetched for free from AniList — no API key
				required.
			</p>
		</div>
	);
