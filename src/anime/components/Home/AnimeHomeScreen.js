...
import AnimeCustomRowsSection from './AnimeCustomRowsSection';
...
export default function AnimeHomeScreen() {
	...
	const filteredSeries = useMemo(() => {
		...
		return allSeries.filter((s) => {
			const title = (metadata[s.id]?.title || s.name).toLowerCase();
			const genres = (metadata[s.id]?.genres || []).join(' ').toLowerCase();
			return title.includes(searchQuery) || genres.includes(searchQuery);
		});
	}, [allSeries, searchQuery, metadata]);
	...
	if (!library || allSeries.length === 0) {
		return (
			<EmptyState
				onScan={scanLibrary}
				onSettings={() => navigate('/settings/anime')}
	...
	return (
		<div className="pb-16">
			...
			<div className="relative z-10 pt-2">
				{/* Fixed top row — recently watched (always rendered, shows progress) */}
				<AnimeRow
					title="Recently Watched"
					series={recentlyWatched}
					showProgress={true}
					emptyMessage="No recently played anime yet. Start watching to see progress here!"
				/>
...
function EmptyState({ onScan, onSettings, scanning, settings }) {
	const hasSourceDirs = settings?.animeSourceDirs?.length > 0;
	return (
		<div className="flex flex-col items-center justify-center h-full gap-6 text-center px-6">
			<div className="text-6xl opacity-20">⛩️</div>
			<div>
				<h2 className="text-2xl font-bold text-white mb-2">
					{hasSourceDirs ? 'Library is empty' : 'Welcome to Anime'}
				</h2>
				<p className="text-gray-400 max-w-md">
					{hasSourceDirs
						? 'No anime was found in your configured directories.'
						: 'Add your anime media directories in Settings, then scan to build your library.'}
				</p>
			</div>
